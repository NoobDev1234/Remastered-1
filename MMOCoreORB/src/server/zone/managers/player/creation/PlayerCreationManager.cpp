					...
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 720000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 20 minutes. Repeat attempts prior to 20 minutes elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
