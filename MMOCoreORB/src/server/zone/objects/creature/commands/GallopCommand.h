...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!checkStateMask(creature)){
			creature->sendSystemMessage("Invalid State");
			return INVALIDSTATE;
		}

		if (!checkInvalidLocomotions(creature)){
			creature->sendSystemMessage("Invalid Locomotion");	
		...
		if (mount == NULL){
			creature->sendSystemMessage("Mount is null");	
			return GENERALERROR;
		}
		...
		if (petManager == NULL || pcd == NULL){
			creature->sendSystemMessage("PetMgror pcd is null");	
			return GENERALERROR;
		}

		SharedObjectTemplate* objectTemplate = pcd->getObjectTemplate();
		if (objectTemplate == NULL){
			creature->sendSystemMessage("Obj Template is null");	
			return GENERALERROR;
		}
