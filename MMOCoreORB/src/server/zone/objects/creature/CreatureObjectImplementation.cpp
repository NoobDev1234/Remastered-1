...
bool CreatureObjectImplementation::isAttackableBy(TangibleObject* object, bool bypassDeadCheck) {
	...
	if (getFactionStatus() == FactionStatus::COVERT || getFaction() == 0)
		return false;
...
bool CreatureObjectImplementation::isHealableBy(CreatureObject* object) {
	...
	//msg << "Target ->" << String::valueOf(targetFactionStatus) << " Current ->" << String::valueOf(currentFactionStatus);
	//sendSystemMessage(msg.toString());

	//msg << "GetFaction ->" << String::valueOf(getFaction()) << " Object GetFaction ->" << String::valueOf(object->getFaction());
	//sendSystemMessage(msg.toString());

	if (getFaction() != object->getFaction() && !(targetFactionStatus == FactionStatus::COVERT)){
		//sendSystemMessage("First check");
		return false;
	}

	if ((targetFactionStatus == FactionStatus::OVERT) && !(currentFactionStatus == FactionStatus::OVERT)){
                //sendSystemMessage("second check");
		return false;
	}
